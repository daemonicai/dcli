...
The `terminal-input` capability defines the raw-mode VT input driver — OS mode shims (`RawModeSession`), the `VtInputParser` state machine, the `KeyEvent`/`PasteEvent`/`ResizeEvent` contract, and lifecycle/restore safety. Modern VT-capable terminals only; the legacy non-VT console input path is not supported.

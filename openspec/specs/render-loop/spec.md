...
The `render-loop` capability defines the actor-model render loop and `Terminal` lifecycle — a single-writer event loop owning all UI state, stdout and the raw-mode session; fire-and-forget command intake; an outbound event stream; coalesced/throttled frames; and guaranteed terminal restore on dispose, crash, or signal.

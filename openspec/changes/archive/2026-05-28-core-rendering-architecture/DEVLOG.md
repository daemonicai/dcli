...
| 15 | Headless test harness (`Dcli.Testing`) | `a649d4b` | 688 | New separate NuGet-packable project `Dcli.Testing` (PackageId=dcli.testing, MPL-2.0, SourceLink + snupkg, one-way ProjectReference to Dcli). Public `HeadlessTerminal` over the real `Terminal.StartCore` (via `InternalsVisibleTo`); scripting drivers `Feed/SendKey/Type/Paste/Resize`; `SettleAsync` wraps internal `LoopEngine.SettleAsync` (no reinvented engine); public `VirtualClock` with off-lock waiter completion. Public immutable `FrameSnapshot` + `FrameSnapshotPrinter` for golden-frame diffing. Single production-code touch: `internal int Autocomplete.SelectedIndex` getter (read-only delegation) for snapshot construction. Harness-based equivalents added alongside existing terminal-free tests (LoopCoalescing + GoldenFrame) — pure parser/renderer unit tests stay direct (their assertions are on internals the public harness can't express; flagged as a follow-up tidy-up). |
...
- **Chunk A — DONE (committed in `a649d4b`).** Created separate `src/Dcli.Testing/` project (NuGet-packable, MPL-2.0, one-way `ProjectReference` to `Dcli`; `InternalsVisibleTo("Dcli.Testing")` added to `Dcli.csproj`). Public `HeadlessTerminal` constructs the real `Terminal` via `Terminal.StartCore` and exposes scripting drivers `Feed(ReadOnlySpan<byte>)` (queued for the real `InputReader` thread; VMIN=0/VTIME=1-shaped `Read` returns 0 when empty, not EOF), `SendKey/Type/Paste/Resize` (post directly to `LoopEngine.InputWriter`; `Type` is `Char(Rune)`-only and docs nudge callers toward `SendKey` for named keys), plus `SettleAsync` (thin wrapper over the existing internal `LoopEngine.SettleAsync` — no shadow engine). Public `VirtualClock` with off-lock waiter completion (existing `FacadeTests` pattern lifted to a single canonical impl). Internal edge fakes: `HeadlessRawModeSession` (no-op), `ScriptedInputByteSource` (`ConcurrentQueue<byte>`), `InMemoryOutputSink` (volatile `LastModel` + `PaintCount`), `FixedSizeSource` (mutable), `ScriptedResizeWatcher` (stored callback + `Fire(cols, rows)`). 10 tests at `tests/Dcli.Tests/Testing/HeadlessTerminalTests.cs` (byte-feed → parsed event uses **F1 `ESC O P`, not Up arrow**, since `NamedKey.Up` is consumed by the editor's history recall and never emits `KeyPressed`; double-`SettleAsync` works around the no-`Feed→reader-barrier` race — a latent gap flagged for a future change). **Reviewer-approved (PASS, 7 non-blocking nits).** 665 → 675 tests.
- **Chunk B — DONE (committed in `a649d4b`).** Public immutable `FrameSnapshot` (record; `LiveWindowRows`/`FixedRegionRows`/`Caret`/`IsCursorVisible`/`Size`/`NewlyCommittedRows` + `OverlayDescriptor { Kind, SelectedIndex, VisibleRowCount, InputText, IsSecret }`); defensive `Line[]` copies on construction to prevent aliasing with future paints. Public `FrameSnapshotPrinter` (stable golden-frame format: ASCII-bordered box + horizon line + caret/overlay trailer). `HeadlessTerminal.Snapshot` getter; empty pre-paint snapshot is safe (does not throw). NuGet polish on `Dcli.Testing.csproj` (SourceLink, snupkg, README + release notes); `src/Dcli.Testing/README.md` ships with a hello-world example. Single production-code touch: `internal int Autocomplete.SelectedIndex => _list.SelectedIndex;` — read-only accessor for Snapshot construction; no widening of mutator surface. Retargeting (§15.6): added harness-based equivalents (`LoopCoalescingHarnessTests.cs` — 4 tests; `GoldenFrameTests.cs` — 2 tests), left existing direct tests in place where they assert on internals (`DirtyCommand`/`WaitCount`/`OutboundEvents.Count`/`ThreadCapturingCommand`) the public harness cannot express — a softer reading of "retarget" than Decision 12's "run on the same harness" wording; flagged as a future tidy-up rather than blocker. Chunk-A nits folded in: `SettleAsync` XML doc qualifies the `Feed`/InputReader race; `_disposed` dead field removed; `ScriptedResizeWatcher.Dispose` doc clarified; `Type(string)` doc adds named-keys advice. **Reviewer-approved (PASS, 4 non-blocking nits — all observational follow-ups).** `dotnet pack -c Release` produces `dcli.0.1.0-rc.1.{nupkg,snupkg}` + `dcli.testing.0.1.0-rc.1.{nupkg,snupkg}` cleanly. 675 → 688 tests.
